    ...
    switch (chunk_common->type) {
      case SCTP_DATA:
        ...
        if (ntohl(data_chunk->ppid) == DATA_CHANNEL_PPID_CONTROL && data_chunk->data[0] == DATA_CHANNEL_OPEN) {
          uint16_t dc_sid = ntohs(data_chunk->sid);
          LOGI("DATA_CHANNEL_OPEN received on sid=%d", dc_sid);
          sctp_parse_data_channel_open(sctp, dc_sid,
              (char*)data_chunk->data,
              ntohs(data_chunk->length) - sizeof(SctpDataChunk));

          // Send DATA_CHANNEL_ACK (ppid=50, single byte 0x02)
          char ack = DATA_CHANNEL_ACK;
          sctp_outgoing_data(sctp, &ack, 1, PPID_CONTROL, dc_sid);
          LOGI("DATA_CHANNEL_ACK sent on sid=%d", dc_sid);
