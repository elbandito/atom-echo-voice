...
int peer_connection_datachannel_open(PeerConnection* pc, const char* label, uint16_t sid) {
  if (!sctp_is_connected(&pc->sctp)) {
    LOGE("sctp not connected");
    return -1;
  }
  ...
  // DATA_CHANNEL_OPEN: msg_type(1) + channel_type(1) + priority(2) +
  //                    reliability(4) + label_len(2) + protocol_len(2) + label
  size_t msg_len = 12 + label_len;
  char msg[64];
  memset(msg, 0, sizeof(msg));
  msg[0] = 0x03;  // DATA_CHANNEL_OPEN
  msg[1] = 0x00;  // DATA_CHANNEL_RELIABLE
  // priority(2) = 0, reliability(4) = 0 (already zeroed)
  msg[8] = (label_len >> 8) & 0xFF;  // label length (big-endian)
  msg[9] = label_len & 0xFF;
  // protocol length(2) = 0 (already zeroed)
  memcpy(msg + 12, label, label_len);

  LOGI("Sending DATA_CHANNEL_OPEN label=%s sid=%d", label, sid);
  return sctp_outgoing_data(&pc->sctp, msg, msg_len, PPID_CONTROL, sid);
}
